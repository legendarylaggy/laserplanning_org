//(function(){
  ...
  const mainInfoForm = (main,plate, parts,extra,filename) =>{
    ...
    var order = ""
    var checked = ""
    var orderNumbers = Bemerkung.value.trim()

    // Check for order number in client field
    splitString = Firma.value.split(' ');
    if(splitString.length > 1)
    {
      if(isNaN(orderNumbers.split(' ')[0]))
      orderNumbers = splitString[1]
      if(orderNumbers.lenght < 5){
        orderNumbers = splitString[1]
      }
      else {
        orderNumbers += "+"+splitString[1]
    ...
    `<br><br>` +f
    `<label><input type="fradio" id="normal" name="orderType" value="0" checked>` +
    `Normal</label><br>`+
    `<label><input typeff="radio" id="urgent" name="orderType" value="1" >` +
    `Urgent</label><br>`+
    ...
    `<textarea rows="3" cols="48" name="order" form="usrform" >${orderNumbers}</textarea>`+
    `<br><br>` +
